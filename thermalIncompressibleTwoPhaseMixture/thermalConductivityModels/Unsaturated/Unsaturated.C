...
{
    defineTypeNameAndDebug(Unsaturated, 0);
    addToRunTimeSelectionTable(thermalConductivity, Unsaturated, components);
}
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::thermalConductivityModels::Unsaturated::Unsaturated
(
...
:
    ...
    //mvCoeff_(Cv_*rho1())
{
	//Info<< "Phase change relaxation time factors for the Unsaturated model:\n" 
	//	<< "Cc = " << Cc_ << endl
	//	<< "Cv = " << Cv_ << endl;
}
...
Foam::thermalConductivityModels::Unsaturated::k(const Foam::thermalIncompressibleTwoPhaseMixture* titpm) const
{
...
bool Foam::thermalConductivityModels::Unsaturated::read()
{
